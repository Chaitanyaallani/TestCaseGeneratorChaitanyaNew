...
# ══════════════════════════════════════════════════════════════════════════════
# GENERATE TEST CASES — Copilot-quality prompt engineering
# ══════════════════════════════════════════════════════════════════════════════

# System-level prompt baked in (adapted from the Copilot agent markdown)
SYSTEM_PROMPT = """You are a Senior QA Architect generating enterprise UI test cases.

QUALITY RULES (MANDATORY):
- title: Clear, descriptive, action-oriented (e.g. "Verify year format change in incident report template displays correctly")
- prerequisites: ALWAYS include user role, starting screen, required test data (e.g. "User: ignio admin logged in, Incident report template accessible, Test incident data available")
- steps: Numbered 1-5, each step is ONE atomic UI action on a named element (e.g. "1. Navigate to incident management system\\n2. Open an existing incident with date \\"Jan 23, 2026\\"\\n3. Generate incident report using standard template")
- expected: Exact visible UI outcome with screen name and message (e.g. "Incident report displays date with correct year format. Template adheres to new format standard. No formatting errors in report.")
- type: Positive / Negative / Edge
- priority: High (core flows, AC-mapped) / Medium (validations, navigation) / Low (edge, cosmetic)

FORBIDDEN:
- Vague titles like "Test button" or "Check page loads"
- Prerequisites that just say "logged in" without role/screen/data
- Steps without actual UI element names
- Expected results that just say "works correctly" or "error shown"
- Combining multiple flows into one test case

MANDATORY COVERAGE:
Positive: Happy path per flow, per role, successful form submission, navigation between screens
Negative: Missing required fields, invalid data formats, unauthorized access, wrong role, disabled elements
Edge: Empty states, max character limits, special characters, page refresh mid-flow, browser back button, session expiry"""


def _build_gen_prompt(feature_text: str, feature_understand: str,
                      similar_cases: str, num_cases: int, type_rule: str,
                      start_id: int, max_chars: int) -> str:
    """Build a quality-focused prompt that fits within token limits."""

    # Budget: ~40% for system prompt, ~30% for feature, ~15% for RAG, ~15% for instructions
    sys_budget = min(1200, max_chars // 3)
    feat_budget = min(800, max_chars // 4)
    rag_budget = min(400, max_chars // 6)

    sys_part = SYSTEM_PROMPT[:sys_budget]
    fu = (feature_understand or "")[:feat_budget]
    ct = (feature_text or "")[:feat_budget // 2]
    rag = (similar_cases or "")[:rag_budget]
...
EXISTING TEST CASES (avoid duplicates, match this quality level):
{rag}

TASK: Generate exactly {num_cases} test cases. Types: {type_rule}.
IDs start from TC-{start_id:04d}.

Return ONLY a valid JSON array. No text before [ or after ].
Each object: {{"id","title","prerequisites","steps","expected","type","priority","related_tc"}}"""
...
def generate_test_cases(combined_text, feature_understand, similar_cases,
    ...
    if auto_mode:
        target = 20; type_rule = "40% Positive, 35% Negative, 25% Edge"
    else:
        target = num_cases
    ...
    # For large context models (32K), we can generate more per batch
    # For small context models (8K), we batch in groups of 5
    if ctx_window >= 32000:
        batch_size = min(target, 15)
    else:
        batch_size = min(target, 5)
    ...
    while remaining > 0:
        ...
        # Adjust type distribution for this batch
        if remaining == target:
            batch_type_rule = type_rule
        else:
            # Later batches: generate what's needed
            pos_so_far = sum(1 for t in all_cases if "positive" in t.get("type","").lower())
            neg_so_far = sum(1 for t in all_cases if "negative" in t.get("type","").lower())
            edge_so_far = sum(1 for t in all_cases if "edge" in t.get("type","").lower())
            needs = []
            if include_neg and neg_so_far < 3: needs.append("more Negative")
            if include_edge and edge_so_far < 2: needs.append("more Edge")
            if not needs: needs.append("mix of Positive/Negative/Edge")
            batch_type_rule = f"Focus on {', '.join(needs)}"

        prompt = _build_gen_prompt(
            combined_text, feature_understand, similar_cases,
            batch_count, batch_type_rule, current_id, max_chars
        )
        ...
        for attempt in range(3):
            try:
                if attempt == 0:
                    ...
                    fallback = f"""Generate {batch_count} QA test cases as JSON array for:
{(combined_text or '')[:300]}

RULES: Each test case needs detailed title (>50 chars), full prerequisites with user role and screen, numbered steps (1-5) with UI element names, detailed expected result with UI outcome.
Keys: id(TC-{current_id:04d}+), title, prerequisites, steps, expected, type(Positive/Negative/Edge), priority(High/Medium/Low), related_tc(None).
Return ONLY JSON array. ["""
                    raw = call_llm(fallback, api_key, provider_config, max_tokens=2000)
                    if not raw.strip().startswith("["): raw = "[" + raw
                else:
                    # Ultra minimal
                    mini = f"""Create {min(batch_count, 5)} test cases JSON for: {(combined_text or '')[:150]}
Keys: id(TC-{current_id:04d}+), title, prerequisites, steps, expected, type, priority, related_tc
["""
                    raw = call_llm(mini, api_key, provider_config, max_tokens=1500)
                    if not raw.strip().startswith("["): raw = "[" + raw
                ...
                if attempt < 2:
                    st.warning(f"⚠️ Batch attempt {attempt+1}: {str(e)[:120]}. Retrying...")
                else:
                    st.error(f"❌ Batch failed: {str(e)[:150]}")

        if not batch_result:
            break  # Stop batching if a batch completely fails

        all_cases.extend(batch_result)
        current_id += len(batch_result)
        remaining -= len(batch_result)

        # Show progress for multi-batch
        if remaining > 0 and target > batch_size:
            st.info(f"📝 Generated {len(all_cases)}/{target} test cases so far...")
...
with col_left:
    ...
    if st.button("⚡ GENERATE TEST CASES", disabled=not can_run):
        ...
        with st.spinner("🧠 Parsing feature..."):
            fu_prompt = f"""Parse this feature input and extract a structured FEATURE UNDERSTANDING.

Return this exact format:
Feature Name        : [name]
User Roles          : [all roles mentioned - admin, user, viewer, etc.]
Screens / Pages     : [all screens identified]
UI Components       : [buttons, inputs, dropdowns, tables, modals per screen]
